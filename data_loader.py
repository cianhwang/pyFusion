...
class SintelImageFolder(data.Dataset):

    def __init__(self, list_path, dpt_list_path, seq, transform = ComposedTransforms([
                                SeqToPILImage(),
                                SeqRandomHorizontalFlip(),
                                SeqRandomVerticalFlip(),
                                SeqRandomCrop((384, 896)),
                                SeqResize((64, 128)),
                                SeqToTensor()])):
        img_list, dpt_list = make_dataset(list_path, dpt_list_path)
        if len(img_list) == 0:
...
#         dptSeq = torch.stack(dptSeq)
    ...
    def replace_num_in_string(self, string, offset):
        ...
        try:
            ...
            print(string)
        subst = "{:04d}".format(int(matches.group()) + offset)
        result = re.sub(regex, subst, string, 0, re.MULTILINE)
        return result
...
if __name__ == '__main__':
    video_dataset = load_davis_dataset('../datasets/mpi-sintel-sample/sintel_vid_tiny.txt', '../datasets/mpi-sintel-sample/sintel_dpt_tiny.txt')
